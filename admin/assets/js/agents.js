class AgentsManager {
        ...
        container.innerHTML = this.users.map(user => `
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                    <button onclick="agentsManager.viewUser('${user._id}')" class="text-blue-600 hover:text-blue-900">View</button>
                    ${user.isActive ? 
                        `<button onclick="agentsManager.suspendUser('${user._id}')" class="text-red-600 hover:text-red-900">Suspend</button>` :
                        `<button onclick="agentsManager.activateUser('${user._id}')" class="text-green-600 hover:text-green-900">Activate</button>`
                    }
                </td>
            </tr>
        `).join('');
        ...
        container.innerHTML = `
            <div class="flex items-center justify-between px-6 py-3">
                <div class="text-sm text-gray-700">
                    Showing <span class="font-medium">${startItem}</span> to <span class="font-medium">${endItem}</span> of <span class="font-medium">${total}</span> agents
                </div>
                <div class="flex gap-2">
                    <button ${page === 1 ? 'disabled' : ''} onclick="agentsManager.goToPage(${page - 1})" 
                        class="px-3 py-1 border rounded ${page === 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-50'}">
                        Previous
                    </button>
                    ${this.renderPageNumbers(page, pages)}
                    <button ${page === pages ? 'disabled' : ''} onclick="agentsManager.goToPage(${page + 1})" 
    ...
    renderPageNumbers(current, total) {
        ...
        return pages.map(page => {
            ...
            return `<button onclick="agentsManager.goToPage(${page})" 
    ...
    async viewUser(userId) {
        try {
            ...
            if (response.status === 'success' && response.data) {
                ...
                AdminUtils.showToast('Failed to load agent details', 'error');
            }
    ...
    showUserDetailModal(user) {
        ...
        if (!modal) {
            console.error('Agent detail modal not found');
            return;
        }
        ...
        modal.innerHTML = `
            <div class="modal-content bg-white rounded-xl shadow-2xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                <div class="p-6 border-b border-slate-200 flex items-center justify-between sticky top-0 bg-white z-10">
                    <h3 class="text-xl font-bold text-slate-900">Agent Details</h3>
                ...
                <div class="p-6">
                    <div class="flex items-start gap-6 mb-6">
                        ...
                        <div class="flex gap-2">
                            ${isActive ? 
                                `<button onclick="agentsManager.suspendUser('${user._id}'); closeModal('userDetailModal')" class="px-4 py-2 bg-red-600 text-white text-sm font-medium rounded-lg hover:bg-red-700 transition-colors">
                                    <i data-lucide="user-x" class="w-4 h-4 inline mr-1"></i> Suspend
                                </button>` :
                                `<button onclick="agentsManager.activateUser('${user._id}'); closeModal('userDetailModal')" class="px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 transition-colors">
    ...
    async suspendUser(userId) {
        const confirmed = await Swal.fire({
            title: 'Suspend Agent?',
            text: 'This agent will not be able to access their account',
            icon: 'warning',
        ...
        if (confirmed.isConfirmed) {
            try {
                ...
                AdminUtils.showToast('Agent suspended successfully', 'success');
                this.loadUsers();
            } catch (error) {
                AdminUtils.showToast(AdminUtils.parseErrorMessage(error), 'error');
            }
    ...
    async activateUser(userId) {
        try {
            ...
            AdminUtils.showToast('Agent activated successfully', 'success');
            this.loadUsers();
        } catch (error) {
            AdminUtils.showToast(AdminUtils.parseErrorMessage(error), 'error');
        }
